...
public class PersonProfileActivity extends AppCompatActivity {
    ...
    private DatabaseReference profileUserRef, UserRef, FriendRquestRef, FriendsRef;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        FriendRquestRef = FirebaseDatabase.getInstance().getReference().child("FriendRequests");
                             ...
                             FriendsRef.child(receiverUserId).child(senderUserID).child("date").setValue(saveCurrentDate)
                                     .addOnCompleteListener(new OnCompleteListener<Void>() {
                                         ...
                                         public void onComplete(@NonNull Task<Void> task) {
                                             if(task.isSuccessful())
                                             {
                                                 FriendRquestRef.child(senderUserID).child(receiverUserId)
                                                         .removeValue()
                                                         .addOnCompleteListener(new OnCompleteListener<Void>() {
                                                             @Override
                                                             public void onComplete(@NonNull Task<Void> task) {
                                                                 if(task.isSuccessful())
                                                                 {
                                                                     FriendRquestRef.child(receiverUserId).child(senderUserID)
                                                                             .removeValue()
                                                                             .addOnCompleteListener(new OnCompleteListener<Void>() {
                                                                                 @Override
                                                                                 public void onComplete(@NonNull Task<Void> task) {
    ...
    private void MaintananceofButtons() {
        FriendRquestRef.child(senderUserID)
